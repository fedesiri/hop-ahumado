...
async function run(filePath) {
  ...
  const entradasData = entradasSheet
    ? XLSX.utils.sheet_to_json(entradasSheet, { header: 1, defval: "" })
    : [];
  const salidasData = salidasSheet
    ? XLSX.utils.sheet_to_json(salidasSheet, { header: 1, defval: "" })
    : [];
  ...
  const codigosSet = new Set([
    ...catalogRows.map((x) => x.codigo),
    ...stockRows.map((x) => x.codigo),
  ]);
  ...
  // 6) Stock ya actualizado en producto (desde Stock General). Opcional: movimientos de ajuste.
  // 7) Movimientos: Entradas → IN, Salidas → OUT
  const { StockMovementType } = require("@prisma/client");
